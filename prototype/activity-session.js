...
function loadMemos() {
    ...
    const stored = typeof getSessionClassStorage === 'function'
        ? getSessionClassStorage('activity_memos', currentSessionId, null)
        : null;
    ...
    if (stored) {
        ...
        const storageKey = `activity_memos_${classCode}_${currentSessionId}`;
        const localStored = localStorage.getItem(storageKey);
        if (localStored) {
            memos = JSON.parse(localStored);
        } else {
        // 더미 데이터 (첫 로드 시)
        memos = [
            {
...
function saveMemos() {
    ...
    if (typeof setSessionClassStorage === 'function') {
        setSessionClassStorage('activity_memos', currentSessionId, memos);
    } else {
        // 폴백: 클래스 코드 직접 사용
        const classCode = typeof getCurrentClassId === 'function' ? getCurrentClassId() : 'default';
        const storageKey = `activity_memos_${classCode}_${currentSessionId}`;
...
function loadAssignment() {
    ...
    if (typeof getSessionClassStorage === 'function') {
        assignmentData = getSessionClassStorage('assignment', sessionId, null);
        if (assignmentData) {
            renderAssignment();
        }
    } else {
        // 폴백: 클래스 코드 직접 사용
        const classCode = typeof getCurrentClassId === 'function' ? getCurrentClassId() : 'default';
        const saved = localStorage.getItem(`assignment_${classCode}_${sessionId}`);
        if (saved) {
...
function saveAssignment() {
    ...
    if (assignmentData) {
        if (typeof setSessionClassStorage === 'function') {
            setSessionClassStorage('assignment', sessionId, assignmentData);
        } else {
            // 폴백: 클래스 코드 직접 사용
            const classCode = typeof getCurrentClassId === 'function' ? getCurrentClassId() : 'default';
            localStorage.setItem(`assignment_${classCode}_${sessionId}`, JSON.stringify(assignmentData));
        }
    }
}
