...
function loadQuizListForSession(sessionId) {
    // localStorage에서 해당 sessionId의 퀴즈 불러오기 (클래스 코드 기반)
    const storedQuizzes = typeof getSessionClassStorage === 'function'
        ? getSessionClassStorage('session_quizzes', sessionId, [])
        : JSON.parse(localStorage.getItem(`session_quizzes_${typeof getCurrentClassId === 'function' ? getCurrentClassId() : 'default'}_${sessionId}`) || '[]');
...
async function handleSubmitQuiz() {
    ...
    if (sessionId) {
        ...
        // 클래스 코드 기반으로 저장
        const storedResults = typeof getSessionClassStorage === 'function'
            ? getSessionClassStorage('session_quiz_results', sessionId, [])
            : JSON.parse(localStorage.getItem(`session_quiz_results_${typeof getCurrentClassId === 'function' ? getCurrentClassId() : 'default'}_${sessionId}`) || '[]');
        storedResults.push(quizResult);
        if (typeof setSessionClassStorage === 'function') {
            setSessionClassStorage('session_quiz_results', sessionId, storedResults);
        } else {
            // 폴백: 클래스 코드 직접 사용
            const classCode = typeof getCurrentClassId === 'function' ? getCurrentClassId() : 'default';
            localStorage.setItem(`session_quiz_results_${classCode}_${sessionId}`, JSON.stringify(storedResults));
        }
